...
<script>
    ...
    window.saveFlowChartData = function() {
        window.isFlowChatInitialized = false;
        var flowchartDataInstance = window.$flowBuilderInstance.flowchart('getData');
        // Loop through each operator key
        Object.keys(flowchartDataInstance.operators).forEach(function(operatorId) {
            var operator = flowchartDataInstance.operators[operatorId];
            // Check if 'properties' exists and has 'body'
            if (operator.properties && 'body' in operator.properties) {
                delete operator.properties.body; // Remove 'body' safely
            }
        });
        __DataRequest.post("{{ route('vendor.bot_reply.bot_flow_data.write.update') }}", {
            'botFlowUid' : '{{ $botFlowUid }}',
            'flow_chart_data' : flowchartDataInstance
            }, function() {
                window.__isUnsavedContent = false;
        });
    };
                        ...
                        data.operators[element._uid] = {
                            ...
                            properties: {
                                ...
                                body: `<div class="btn-group btn-group-sm"><a class="btn btn-default lw-mobile-view-none" style="padding: 1px;"></a> <a style="display:none;" x-show="isUnsavedContent" @click.prevent="window.unsavedAlert()" title="{{  __tr('Edit') }}" class="btn btn-default btn-sm" href="#"><i class="fa fa-edit"></i> {{  __tr('Edit') }}</a> <a x-show="!isUnsavedContent" data-pre-callback="appFuncs.clearContainer" title="{{  __tr('Edit') }}" class="btn btn-default lw-ajax-link-action" data-response-template="#lwEditBotReplyBody" href="`+__Utils.apiURL("{{ route('vendor.bot_reply.read.update.data', [ 'botReplyIdOrUid']) }}", {'botReplyIdOrUid': element._uid})+`" data-toggle="modal" data-target="#lwEditBotReply"><i class="fa fa-edit"></i> {{  __tr('Edit') }}</a> <a style="display:none;" x-show="isUnsavedContent" @click.prevent="window.unsavedAlert()" title="{{  __tr('Delete') }}" class="btn btn-danger btn-sm" href="#"><i class="fa fa-trash"></i> {{  __tr('Delete') }} </a>
                                    ...
                                    <a class="btn btn-light lw-mobile-view-none" style="padding: 1px;"></a></div><button style="display:none;" class="lw-delete-link-btn lw-operator-link-`+element._uid+` btn btn-warning btn-block btn-sm" @click="window.$flowBuilderInstance.flowchart('deleteSelected');"><i class="fas fa-unlink"></i> {{  __tr('Delete Link') }}</button>`,
                                inputs: {
                                    input: {
                                        label: "→"
                                    },
